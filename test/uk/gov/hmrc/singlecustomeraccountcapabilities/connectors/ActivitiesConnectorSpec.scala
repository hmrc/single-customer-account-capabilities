...
class ActivitiesConnectorSpec extends AsyncWordSpec with Matchers with WireMockHelper with HttpClientSupport with MockitoSugar {
  ...
  "getActivities" must {
    "return the Activity data with valid Nino" in {
      ...
      activitiesConnector.list(nino).map { response =>
...
object ActivitiesConnectorSpec {
  ...
  private val activitiesUrl = s"/individuals/activities/NINO/$nino"
}
